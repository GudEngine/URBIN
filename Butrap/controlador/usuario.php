...
class Usuario
{
    ...
    // Limpiamos el string para evitar inyecciones SQL básicas
		$email = mysqli_real_escape_string($this->conn, $email);
		
		$query = "DELETE FROM usuario WHERE usr_email = '$email'";
		
		if (mysqli_query($this->conn, $query)) {
			// Verificamos si realmente se borró alguien (si el email existía)
			if (mysqli_affected_rows($this->conn) > 0) {
				return json_encode(["success" => true, "message" => "Usuario eliminado correctamente"]);
			} else {
				return json_encode(["success" => false, "error" => "No se encontró ningún usuario con ese email"]);
			}
		} else {
			return json_encode(["success" => false, "error" => "Error en la base de datos: " . mysqli_error($this->conn)]);
		}
	}
